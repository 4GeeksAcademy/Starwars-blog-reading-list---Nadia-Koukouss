...
def get_user_favoritos():
    ...
    favoritos_personajes = FavoritoPersonaje.query.filter_by(
        user_id=user.id).all()
    favoritos_planetas = FavoritoPlaneta.query.filter_by(user_id=user.id).all()

    favoritos = {
        "personajes": [f.serialize() for f in favoritos_personajes],
        "planetas": [f.serialize() for f in favoritos_planetas]
    }
...
def add_favorito_personaje(personaje_id):
    ...
    user = User.query.first()
    favorito = FavoritoPersonaje.query.filter_by(
        user_id=user.id, personaje_id=personaje_id).first()
    if favorito:
...
def add_favorito_planeta(planeta_id):
    ...
    user = User.query.first()
    favorito = FavoritoPlaneta.query.filter_by(
        user_id=user.id, planeta_id=planeta_id).first()
    if favorito:
        return jsonify({"mensaje": "Ya existe en favoritos"}), 400
